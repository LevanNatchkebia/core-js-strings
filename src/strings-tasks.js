...
function getStringLength(value) {
  if (value != null && typeof value === 'string') return value.length;
...
function sumOfCodes(str) {
  if (typeof str === 'string' || str instanceof String) {
    return str.split('').reduce((result, currentCharacter) => {
      return result + currentCharacter.charCodeAt(0);
    }, 0);
  }
  return 0;
}
...
function countVowels(str) {
  ...
  return str.split('').reduce((amount, char) => {
...
function invertCase(str) {
  ...
  const splitedWords = words.map((splitedWord) => {
    if (splitedWord === splitedWord.toUpperCase()) {
      return splitedWord.toLowerCase();
    }

    return splitedWord.toUpperCase();
  });
...
function getCardId(value) {
  ...
  return cardsArry.indexOf(value);
}
